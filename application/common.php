...
function mac_substring($str, $lenth, $start = 0)
{
    ...
    for ($i = 0; $i < $len; $i ++) {
        ...
        if ($n < $start) {
            if (substr($a, 0, 1) == 0) {} else if (substr($a, 0, 3) == 110) {
                $i += 1;
    ...
    switch ($model) {
        ...
        case 'vod/detail':
            $replace_to = [
                ...
                $info['type_1']['type_id'],
                $info['type_1']['type_en'],
                date('Y', $info['vod_time']),
                date('m', $info['vod_time']),
                date('d', $info['vod_time'])
            ];
        ...
        case 'art/detail':
            $replace_to = [
                ...
                $info['type_1']['type_id'],
                $info['type_1']['type_en'],
                date('Y', $info['art_time']),
                date('m', $info['art_time']),
                date('d', $info['art_time'])
            ];
...
{
    $tab = 'vod';
    if ($info['type_mid'] == 1) {} else if ($info['type_mid'] == 2) {
        $tab = 'art';
    }
 ...
 * *类型 1.佣金提现， 2.佣金派发
 */
function mac_withdraw_type($val)
{
    switch ($val) {
        case 1:
            return '佣金提现';
 ...
 * *状态 0.申请中 1.通过 2.拒绝
 */
function mac_withdraw_status($val)
{
    switch ($val) {
        case 1:
            return '通过';
...
function mac_return_proxy_name($proxy_id)
{
    ...
    $res = model('Proxy')->findData($where, 'name');
