...
class User extends Base
{
    ...
    public function infoData($where, $field = '*', $group = true)
    {
    ...
    public function register($param)
    {
        ...
        if ($param['sendPhone'] === true) {
            
            $data['user_phone'] = $param['user_phone'];
            
            if (! preg_match("/^1\d{10}$/", $data['user_phone'])) {                
                return [
                    'code' => 1005,
                    'msg' => '手机号格式错误'
                ];
            ...
            // 手机号
            $w['phone'] = $data['user_phone'];
            // 短信验证码
            $w['code'] = $param['phone_code'];
            // 过期时间判断
            $w['exp_time'] = [
                '>',
                time()
            ];
            $res = model('SendPhone')->findData($w,'id');           
            if($res['code'] > 1){
                return [
                    'code' => 1005,
                    'msg' => '手机验证码错误'
            ...
            // 判断手机是否存在
            $row = $this->where('user_phone', $data['user_phone'])->find();
            if (! empty($row)) {
                return [
                    'code' => 1005,
                    'msg' => '手机号已被注册，请更换'
        ...
        // 手机号注册 不需要 图像验证码
        if (empty($param['sendPhone']) && ! captcha_check($data['verify'])) {
            return [
                'code' => 1003,
                'msg' => '验证码错误'
            ];
        ...
        if($data['user_phone']){
            $fields['user_phone'] = $data['user_phone'];
        }        
    ...
    public function bind($param)
    {
        ...
        if (! in_array($param['ac'], [
            'email',
            'phone'
        ]) || empty($param['to']) || empty($param['code'])) {
            return [
                'code' => 2001,
                'msg' => '参数错误'
            ];
